...
def plot_dataset_samples_1d(
    ...
    with plot_config(plot_config_kwargs):
        ...
        for i in range(n_samples):
            #print("randint ->", np.random.randint(len(dataset)))
            x, y = dataset[np.random.randint(len(dataset))]
            #print("i -> {}, X -> {}".format(i, x.numpy()))
    ...
    **kwargs,
    ...
    with plot_config(plot_config_kwargs):
        ...
        if (xx is not None and yy is not None):
            X_cntxt = xx
            Y_cntxt = yy
            _,_,X_trgt, Y_trgt = get_cntxt_trgt(X, Y)
        elif( data is not None):
            X_cntxt, Y_cntxt, X_trgt, Y_trgt = get_cntxt_trgt(X, Y, context_indcs=data) ##### __call__ GettrgtCntxt
        else:
            X_cntxt, Y_cntxt, X_trgt, Y_trgt = get_cntxt_trgt(X, Y)  ##### __call__ GettrgtCntxt
        #print("Shape context x: {} y: {} target x: {} y: {}".format(xx, yy, X_trgt.shape, Y_trgt.shape))
        ...
        if generator is not None:
            X_cntxt_plot = X_cntxt.numpy()[0]#rescale_range(X_cntxt, (-1, 1), train_min_max).numpy()[0]
            ...
            #print("mean: {} and std:{}".format(mean_y.shape, std_y.shape))
...
def _plot_posterior_predefined_cntxt(
    ...
    **kwargs,
    ...
    if is_conditioned:
        ...
        #X_cntxt_plot = rescale_range(X_cntxt_plot, (-1, 1), train_min_max) #######ojo train_min_max
    print("contxt ->", X_cntxt_plot)
    ...
    for i, (mean_y, std_y) in enumerate(gen_p_y_pred(model, X_cntxt, Y_cntxt, X_trgt, n_samples)):
        if not is_smooth:
            kwargs["linestyle"] = ""
    ...
    ax.set_xlim(x_min, x_max)#float(x_max))
